...
export const createHandler = async argv => {
    ...
    try {
        if (projectName) {
			...
			copySync(join(__dirname, '..', 'templates'), join(projectDir, '.templates'));
        }
    } catch (error) {
		console.error(error);
		return;
	...
	if (argv.y) {
		...
		options = Object.assign(argv, await inquirer.prompt(createPromptItems(argv)));
	}

	// start creating project according to options
	const tplPkg = readFileSync(join(projectDir, '.templates/package.json'), 'utf8');
	...
	writeFileSync(join(projectDir, 'package.json'), template(tplPkg)({ projectName }));

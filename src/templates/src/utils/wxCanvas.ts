...
// 绘制圆
export function drawCircle<T extends ICtx & IFillStyle & IArc>(options: T) {
    const { ctx, color, r, x, y } = options;
    ctx.save();
    ctx.setFillStyle(color);
    ctx.beginPath();
...
export function drawCircleImage(options: any) {
    const { ctx, imageResource, r, x, y, color } = options;
    ctx.save();
    ctx.beginPath();
    ctx.arc(x, y, r, 0, 2 * Math.PI);
    ctx.setFillStyle(color);
...
// 目前只考虑 加重字段在 文本中，不考虑在头|尾的情况 且 仅居中类型
export function fillTextWidthStrongStyle<T extends ICtx & IFillStyle & IText & IFontSize & ITextAlign & ITextBaseLine & IBold & IStrongText & ICanvas>(options: T) {
    const { ctx, text, fontSize, color, x, y, align, baseline, isBold, strongText, canvas } = options;
    ...
    if (strongText && ~text.indexOf(strongText.text)) {
        ...
        const strongW = t.strLen(strongText.text) * strongChar;
        ...
        fillText(Object.assign({}, options, {
            ...
            x: (+canvas.width - strongW) / 2 - 10,
            align: 'right'
        }));

        fillText(Object.assign({}, options, {
            text: simpeTextList[1],
            x: (+canvas.width + strongW) / 2 + 13,
            align: 'left'
        }));
    }
}
...
// 圆角矩形
export function drawRoundedRect<T extends ICtx & IArc & IRect & IFillStyle & IShadow>(options: T) {
